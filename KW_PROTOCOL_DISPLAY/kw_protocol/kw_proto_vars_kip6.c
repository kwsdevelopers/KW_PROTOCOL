...
{
  p_vars = &vars_kip6_interface;   
}


int32_t kw_vars_kip6_set_alarm_status( ENUM_ALARM_NAME alarm_name, uint32_t val )
{
...
int32_t kw_vars_kip6_get_setpoint( ENUM_SETPOINT_NAME setpoint_name, value_pack_t *p_value )
{
  value_pack_t result;
  ...
  if( setpoint_name >= SETPOINT_MAX )
    ...
    case SETPOINT_KIP6_IP_ADR:                               // Уставка IP адрес устройства
      p_value->val_byte[0] = 1;
      p_value->val_byte[1] = 2;
      p_value->val_byte[2] = 3;
      p_value->val_byte[3] = 4;

      break;    
    case SETPOINT_KIP6_IP_MASK:                              // Уставка IP маска сети
      p_value->val_byte[0] = 5;
      p_value->val_byte[1] = 6;
      p_value->val_byte[2] = 7;
      p_value->val_byte[3] = 8;

      break;    
    case SETPOINT_KIP6_IP_GATE:                              // Уставка IP арес шлюза сети
      p_value->val_byte[0] = 9;
      p_value->val_byte[1] = 10;
      p_value->val_byte[2] = 11;
      p_value->val_byte[3] = 12;
...
int32_t kw_vars_kip6_set_setpoint( ENUM_SETPOINT_NAME setpoint_name, value_pack_t val )
{  
  value_pack_t result;
...
int32_t  kw_vars_kip6_set_value( ENUM_VALUE_NAME value_name, value_pack_t value )
{
  value_pack_t result;
...
int32_t kw_vars_kip6_get_array( ENUM_ARRAY_NAME array_name, uint8_t *p_buf, uint32_t buf_sz )
{
...
int32_t kw_vars_kip6_set_array( ENUM_ARRAY_NAME array_name, uint8_t *p_array, uint32_t array_sz )
{
  if( array_name >= ARRAY_MAX )
    return -1;
...
int32_t kw_vars_kip6_get_date_time( prop_dt_t *p_date_time )
{ 
  // это время для посылки на шину каждую секунду
...
int32_t kw_vars_kip6_set_date_time( prop_dt_t *p_date_time )
{
  // время для установки в RTC
...
kw_proto_vars_interface_t vars_kip6_interface = 
{
  kw_vars_kip6_init,
  0,
  kw_vars_kip6_set_alarm_status,                                // не надо устанавливать аварии
  kw_vars_kip6_get_setpoint,
  kw_vars_kip6_set_setpoint,
  0,
  kw_vars_kip6_set_value,                                // не надо устанавливать значения переменных
  kw_vars_kip6_get_array,
  kw_vars_kip6_set_array,
  kw_vars_kip6_get_date_time,
  kw_vars_kip6_set_date_time
};
